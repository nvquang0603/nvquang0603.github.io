		...
		<div class="product">
			...
			<div class="row product-row">
				...
				foreach ($mostViewProducts as $key => $feature) {
					...
					<div class="col-md-3 single-card">
						<div class="card">
							<a href="<?= $siteUrl?>product_detail.php?id=<?= $feature['id']?>"><?php inOutStock($feature['status']) ?></a>
							<a href="<?= $siteUrl?>product_detail.php?id=<?= $feature['id']?>"><img class="card-img-top" src="<?php echo $feature['image']?>" alt="Card image cap"></a>
							<div class="card-body">
								<a href="<?= $siteUrl?>product_detail.php?id=<?= $feature['id']?>"><h5 class="card-title"><?php echo $feature['product_name'] ?></h5></a>
								<p class="card-text list-price"><?php echo $feature['list_price'] ?></p>
								<p class="cart-text sale-price"><?php echo $feature['sell_price']; ?></p>
								<a href="<?= $siteUrl?>product_detail.php?id=<?= $feature['id']?>" class="btn btn-primary">Xem chi tiết</a>
							</div>
						</div>
					</div>
					<?php
